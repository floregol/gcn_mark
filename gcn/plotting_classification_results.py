...
result_path = 'results/classification/softmax/'
...
plot_path = 'plots/classification/'
intitial_files = 'initial_neigh.pk'
with open(result_path + intitial_files, 'rb') as f:
    initial_y = pk.load(f, encoding='latin1')
gcn_classifier = np.argmax(initial_y[1708:2608], axis=1)
gcn_good = np.where(gcn_classifier == label_ground_truth)
gcn_not = np.where(gcn_classifier != label_ground_truth)
